...
const EstadoBadge: React.FC<{ estado: string }> = ({ estado }) => (
  <span className={`badge ${getEstadoBadgeClass(estado)} capitalize`}>{estado}</span>
)
  ...
  <div className="bg-gradient-to-r from-primary to-primary-focus px-3 sm:px-6 py-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
    ...
    <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-4 w-full sm:w-auto">
      ...
      <div className="join w-full sm:w-auto">
        ...
        />
        {searchQuery && (
          <button onClick={() => onSearchChange({ target: { value: "" } } as any)} className="btn btn-square join-item">
      ...
      <div className="flex items-center gap-2 w-full sm:w-auto">
        <div className="dropdown dropdown-end">
          ...
          <ul tabIndex={0} className="dropdown-content menu p-2 shadow bg-base-200 rounded-box w-48 sm:w-52 z-[9999]">
...
const RowActions: React.FC<{
  ...
  onComentarioCliente?: (tipo: "cliente" | "producto", pedidoId: string, comentario: string) => void
  onComentarioProducto?: (tipo: "cliente" | "producto", pedidoId: string, comentario: string) => void
}> = ({
  row,
  onRemitir,
  onEditar,
  ...
  return (
    <div className="dropdown dropdown-end">
      ...
      <ul tabIndex={0} className="dropdown-content menu p-2 shadow bg-base-100 rounded-box w-48 z-[9999]">
        {!isVendedor && (
          <>
            <li>
              <button onClick={() => onRemitir(row)} className="flex items-center gap-2">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
            ...
            {row.estado?.toLowerCase() !== "entregado" && row.disponible === "Disponible" && (
              <li>
                <button onClick={() => onCambiarEstado(row.id, "entregado")} className="flex items-center gap-2">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {!isVendedor && (
          <>
            <li>
              <button onClick={() => onEliminar(row.id)} className="flex items-center gap-2 text-error">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
            ...
            <li>
              <button
                onClick={() => onComentarioCliente?.("cliente", row.id, row.comentario_cliente || "")}
              ...
              >
                ...
                {row.comentario_cliente ? "Editar comentario cliente" : "Añadir comentario cliente"}
              </button>
            </li>
            {row.remitos && row.remitos.length > 0 && (
              <li>
                <button onClick={() => onVerComprobante(row.remitos![0].url)} className="flex items-center gap-2">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
const TableCell: React.FC<{
  ...
  const renderCellContent = () => {
    ...
    if (col.key === "disponible") {
      return <span className={`badge ${getEstadoBadgeClass(row[col.key] as string)} capitalize`}>{row[col.key]}</span>
    }
    if (col.key === "remito") {
      return (
        <div className="flex items-center gap-1">
          <div className="avatar placeholder">
            <div className="bg-primary text-primary-content rounded-full w-5 h-5 flex items-center justify-center">
              <span className="text-xs font-bold text-primary-content">#{row[col.key]}</span>
            </div>
          </div>
      ...
      return (
        <div className="flex items-center gap-2">
          <div className="badge badge-outline badge-primary">
            <span className="font-bold text-base-content text-sm">{row[col.key]}</span>
          </div>
          {row.comentario_cliente && (
            <div className="tooltip tooltip-left" data-tip={row.comentario_cliente}>
      ...
      return (
        <div className="flex items-center gap-2">
          ...
          {row.comentario_producto && (
            <div className="tooltip tooltip-left" data-tip={row.comentario_producto}>
              <div className="w-2 h-2 bg-info rounded-full"></div>
            </div>
          )}
        </div>
...
const TablePedidos: React.FC = () => {
  ...
  const { successMessage, setSuccessMessage, errorMessage, setErrorMessage, showStockError, setShowStockError } =
    messageState

  // Estado para el modal de comentarios
  const [showComentarioModal, setShowComentarioModal] = useState(false)
  const [comentarioTipo, setComentarioTipo] = useState<"cliente" | "producto">("cliente")
  ...
  const toggleExpand = useCallback(
    (remitoId: string) => {
      setExpandedRows((prev) => (prev.includes(remitoId) ? prev.filter((id) => id !== remitoId) : [...prev, remitoId]))
    },
    [setExpandedRows],
  )
  ...
  // Cambiar estado: ahora verifica stock antes de permitir entregar
    ...
    (pedidoId: string, nuevoEstado: string) => {
      ...
      if (pedido && pedido.productos) {
        ...
        if (productosPendientes.length > 0) {
          // Store the pending products details
          setPendingProducts(productosPendientes)
          setShowStockError(true)
          return
        }
      }
      // Si no hay pendiente, continuar con el flujo normal
      setAccionActual({ pedidoId, tipo: "entregar" })
      setModalOpen(true)
    },
    [setAccionActual, setModalOpen, pedidos, setShowStockError],
  )
...
const sortedPedidos = useMemo(() => {
  ...
  const nextRemito = useMemo(() => {
    const maxRemito = pedidos.length > 0 ? Math.max(...pedidos.map((pedido) => Number(pedido.remito) || 0)) : 0
    return maxRemito + 1
  }, [pedidos])
  ...
  return (
    <div className="h-[calc(100vh-8rem)] flex flex-col">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full mx-4 p-6">
            <div className="flex items-center mb-4">
              ...
              <div className="ml-3">
                <h3 className="text-lg font-medium text-gray-900">No se puede entregar el pedido</h3>
              </div>
            </div>

            <div className="mb-4">
              <p className="text-sm text-gray-600 mb-3">Los siguientes productos tienen stock pendiente:</p>
                          ...
                          {col.key === "cantidadM2" && (
                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path
                                ...
                                strokeLinejoin="round"
                                strokeWidth={2}
                                d="M9 19v-6a2 2 0 110-2 2 2 0 010 2zm0 7a1 1 0 110-2 1 1 0 010 2zm0 7a1 1 0 110-2 1 1 0 010 2z"
                              />
                            </svg>
                          )}
                          {col.key === "materiales" && (
                  ...
                  return (
                    <React.Fragment key={rowIndex}>
                      <tr
                        ...
                        onClick={() => tieneMasDeUnProducto && toggleExpand(row.remito)}
                      >
                        {visibleColumns
                          .filter((col) => col.visible)
                          .map((col) =>
                      ...
                      {isExpanded &&
                        row.productos?.slice(1).map((prod: any, prodIndex: number) => (
                          <tr key={`extra-${rowIndex}-${prodIndex}`} className="bg-base-200 text-sm">
                            {visibleColumns
                              .filter((col) => col.visible)
                              .map((col) => {
                                if (col.key === "acciones") {
                                  return (
                                    <td key={col.key} className="px-2 py-1 sm:px-4 sm:py-2 italic text-gray-500"></td>
                                  )
                                }
                                ...
                                switch (col.key) {
                                  ...
                                  case "valorM2":
                                    ...
                                    break
                                  case "disponible":
                                    content = prod.estado_stock
                                    break
                                }

                                return (
                                  <td key={col.key} className="px-2 py-1 sm:px-4 sm:py-2 italic text-gray-500">
                                    {content}
                                  </td>
                                )
                              })}
      ...
      {remitoData && <RemitoModal remitoData={remitoData} onClose={() => setRemitoData(null)} />}
      ...
      {pedidoParaEditar && (
        <Modal isOpen={!!pedidoParaEditar} onClose={() => setPedidoParaEditar(null)}>
          <NuevoPedido
            onClose={() => {
              setIsModalOpen(false)
      ...
      <ComentarioModal
        ...
        titulo={`Comentario ${comentarioTipo === "cliente" ? "del Cliente" : "del Producto"}`}
